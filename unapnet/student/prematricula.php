<?php
	...
	$tCurwat = "unapnet.curmat{$sUser['cod_car']}2005";
	$tEstutor = "unapnet.estutut{$sUser['cod_car']}2005";
	$tPlan = "unapnet.curso";

	$xSerdata = mysql_connect($sConedb['host'], $sConedb['user'], $sConedb['passwd']);
	
	$vQuery = "Select pln_est from $tEstutor where num_mat = '{$sUser['codigo']}' and per_aca = '02'";
	...
	$vSubheader = array('Especialidad', 'Plan', 'Nivel', 'Grupo', 'Mod', 'C');

	$vQuery = "Select cod_esp, pln_est, niv_est, sec_gru, mod_mat, tot_crd from $tEstuwat where num_mat = '{$sUser['codigo']}' and per_aca = '02'";
	$cEstuwat = mysql_query($vQuery, $xSerdata);
	if($aEstuwat = mysql_fetch_array($cEstuwat))
	{
		$bEst_mat = TRUE;
		$vCont = 0;
		$vTitle = "Pre-Matrícula Realizada 2004-II";
		$vBody = '';
		$vSubody = '';

		$vSubody[$vCont] = array($sEspecial[$aEstuwat['cod_esp']], $aEstuwat['pln_est'], $aEstuwat['niv_est'], $sGrupo[$aEstuwat['sec_gru']], 
			$sModmat[$aEstuwat['mod_mat']], $aEstuwat['tot_crd']);
		
		$vQuery = "Select $tPlan.niv_est, $tPlan.sem_anu, $tPlan.nom_cur, $tCurwat.sec_gru, $tCurwat.mod_mat, $tCurwat.tur_est, $tPlan.crd_cur ";
		$vQuery .= "from $tCurwat left join $tPlan on $tCurwat.pln_est = $tPlan.pln_est and $tCurwat.cod_cur = $tPlan.cod_cur ";
		$vQuery .= "where $tCurwat.num_mat = '{$sUser['codigo']}' and per_aca = '02'";
		
		$cCurwat = mysql_query($vQuery, $xSerdata);
		while($aCurwat = mysql_fetch_array($cCurwat))
		{
			$vBody[$vCont] = array($sNivel[$aCurwat['niv_est']], $aCurwat['sem_anu'], ucwords(strtolower($aCurwat['nom_cur'])), 
				$sGrupo[$aCurwat['sec_gru']], $sModmat[$aCurwat['mod_mat']], $sTurno[$aCurwat['tur_est']], $aCurwat['crd_cur']);
			$vCont++;
			
			$sCursopdf[$vConc]['num_cur'] = $vConc+1;
			$sCursopdf[$vConc]['niv_est'] = $sNivel[$aCurwat['niv_est']];
			$sCursopdf[$vConc]['sem_anu'] = $sSemestre[$aCurwat['sem_anu']];
			$sCursopdf[$vConc]['nom_cur'] = ucwords(strtolower($aCurwat['nom_cur']));
			$sCursopdf[$vConc]['sec_gru'] = $sGrupo[$aCurwat['sec_gru']];
			$sCursopdf[$vConc]['mod_mat'] = $sModmat[$aCurwat['mod_mat']];
			$sCursopdf[$vConc]['tur_est'] = $sTurno[$aCurwat['tur_est']];
			$sCursopdf[$vConc]['crd_cur'] = $aCurwat['crd_cur'];
			$vConc++;
		}
...
<!-- fwtable fwsrc="estdoc.png" fwbase="index.jpg" fwstyle="Dreamweaver" fwdocid = "377833132" fwnested="0" -->
  ...
  <tr>
   ...
   <td align="center" valign="top"><span class="celdapar"><a href="prnficha.php" title="Presentaci&oacute;n Preliminar">Imprimir</a></span>     <?=$vHtml?> <? if(!$bEst_mat) { ?>   <form action="<?=$vFile.'?'.SID?>" method="post" name="fMatri" id="fMatri">
     <table border="0" cellpadding="0" cellspacing="0">
       ...
       <tr>
         ...
         <td align="center" background="../../images/ventana_r1_c2.jpg" class="tlargebb"> Pre-matricula 2004-II </td>
         <td><img name="ventana_r1_c4" src="../../images/ventana_r1_c4.jpg" width="11" height="29" border="0" alt=""></td>
       </tr>
       <tr>
         <td background="../../images/ventana_r2_c1.jpg"></td>
         <td align="center" background="../../images/ventana_r2_c2.jpg" class="tnormalbn"><input type="submit" name="Submit" value="Matricular cursos de Tutor&iacute;a"></td>
